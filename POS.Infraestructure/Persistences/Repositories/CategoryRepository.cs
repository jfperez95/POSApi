...
namespace POS.Infraestructure.Persistences.Repositories
{
    public class CategoryRepository : GenericRepository<Category>, ICategoryRepository
    {
        ...
        public async Task<BaseEntityResponse<Category>> ListCategories(BaseFiltersRequest filters)
        {
            ...
            if (filters.StateFilter is not null && !string.IsNullOrEmpty(filters.EndDate))
            {
            ...
            response.Items = await Ordening(filters, categories, !(bool)filters.Download!).ToListAsync();
            return response;
        }
    }
}
